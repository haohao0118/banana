// GM 后台 API · Cloudflare Pages Function
// 环境变量在 Cloudflare Pages → Settings → Environment variables 里设置：
...
export async function onRequestPost(context) {
  ...
  const GM_PASSWORD          = env.GM_PASSWORD;

  const CORS = { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' };

  function ok(body)            { return new Response(JSON.stringify(body),        { status: 200, headers: CORS }); }
